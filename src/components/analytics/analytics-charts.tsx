...
export function ReviewComparisonChart({
  ...
  const chartData = data.map((d) => ({
    stage: d.stage,
    average: d.average !== null ? Math.round(d.average * 10) / 10 : 0,
    count: d.count,
      ...
      <LineChart data={chartData} margin={{ top: 8, right: 16, left: 8, bottom: 24 }}>
        ...
        <Tooltip
          formatter={(value, _name, props) => {
            const payload = props?.payload as { hasData?: boolean; count?: number } | undefined;
            if (!payload?.hasData) return ["—", "Average"];
