...
function averageReviewTotal(students: StudentWithMarks[], reviewNumber: 1 | 2 | 3): number | null {
  const totals: number[] = [];
  for (const student of students) {
    const record = buildStudentMarksRecord(student);
    const row = record.reviews[reviewNumber];
    if (!row) continue;
    const total = extractFinalTotal(row);
...
function buildSkillAverages(students: StudentWithMarks[]): SkillAverage[] {
  const records = students.map(buildStudentMarksRecord);
...
export function buildDashboardAnalytics(
  ...
  const distribution = buildMarksDistributionData(students);
  ...
  const review1Avg = averageReviewTotal(students, 1);
  const review2Avg = averageReviewTotal(students, 2);
  const review3Avg = averageReviewTotal(students, 3);
  ...
  return {
    ...
    skillAverages: buildSkillAverages(students),
    internship: {
      withInternship,
      withoutInternship,
      completionPercent:
...
export function buildStudentProfileAnalytics(
  ...
  const timeline = ([1, 2, 3] as const).map((n) => ({
    stage: `Review ${n}`,
    marks: record.reviews[n] ? extractFinalTotal(record.reviews[n]!) : null,
  ...
  timeline.push({
    stage: "Final Marks",
    marks: extractFinalTotal(record.finalRow),
  ...
  return {
    ...
    finalTotal: extractFinalTotal(record.finalRow),
